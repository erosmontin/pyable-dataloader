"""
Example 2: Segmentation Training with Data Augmentation

This example demonstrates the same segmentation model as Example 1,
but with data augmentation applied during training.
"""
...
def train_segmentation_with_augmentation():
    """Train the segmentation model with data augmentation."""
    ...
    augmentation_transforms = Compose([
        ...
        RandomRotation(rotation_range=[[-10, 10], [-10, 10], [-5, 5]], prob=0.8),
        RandomTranslation(translation_range=[[-3, 3], [-3, 3], [-2, 2]], prob=0.6)
    ])

    print("Setting up dataset with augmentation...")
    dataset = PyableDataset(
        manifest=str(manifest_path),
        target_size=[32, 32, 32],
        target_spacing=1.0,
        transforms=augmentation_transforms,  # Apply augmentation
        return_meta=False
    )
    ...
    print(f"Training with augmentation on {device}...")
    ...
    # Save model
    torch.save(model.state_dict(), 'segmentation_model_augmented.pth')
    print("Model saved as 'segmentation_model_augmented.pth'")


if __name__ == '__main__':
    train_segmentation_with_augmentation()